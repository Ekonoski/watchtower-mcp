...
def _frame(mf_shift=0.0, stale_cross=False, rsi_fade=False,
    ...
    (37/18, the UNH green look); Williams %R pinned to −85 then curling
    up through −71; MACD histogram carries two sub-zero troughs, the
    second higher (the full-stack bonus)."""
    ...
    pctr = np.full(N, -50.0)
    if not pctr_flat:
        pctr[-8:] = [-70, -80, -85, -83, -80, -77, -74, -71]
...
def main():
    ...
    # 4e) %R leg (v8): a %R that never reached the floor has nothing to
    # curl out of — refused, however washed the flow is.
