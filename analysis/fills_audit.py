...
entry row and one exit row into `fills_audit`, on the SAME cursor /
transaction as the paper_trades INSERT / exit UPDATE. If the audit
...
def _insert(cur, *, paper_trade_id, book, ticker, event, got_px,
    ...
    cur.execute(INSERT_SQL, (
        ...
        _jsonb(bar), _jsonb(evidence), provenance,
    ))
...
def gap_report(trades, audits):
    ...
    trades: iterable of {id, book, exited_at}
    audits: iterable of {paper_trade_id, event}

    Returns {book: {n_trades, n_open, n_closed, missing_entry,
                    missing_exit, complete}}. No pooled score — a
    ...
    for t in trades:
        ...
        slot = out.setdefault(book, {
            ...
            "missing_entry": 0, "missing_exit": 0, "complete": 0,
        })
        slot["n_trades"] += 1
